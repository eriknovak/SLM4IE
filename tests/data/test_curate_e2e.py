...
def _write_pretrain_config(path: Path, input_dir: Path, output_dir: Path) -> None:
    ...
    repo's real `configs/data/curate.yaml`.
...
def test_subset_run_then_all_is_incremental(tmp_path: Path) -> None:
    ...
    only its scoped stages ran (no corpus dedup/statistics). Then runs `--all`
    and asserts `alfa`'s scoped work is skipped, `beta` is processed
    through the scoped stages, and the corpus dedup/statistics stages run
    ...
    assert not (out_dir / "05_exact_dedup" / ".complete").exists()
    assert not (out_dir / "07_statistics" / ".complete").exists()
    ...
    assert (out_dir / "05_exact_dedup" / ".complete").exists()
    assert (out_dir / "07_statistics" / ".complete").exists()
