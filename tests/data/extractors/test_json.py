...
class TestJsonExtractor:
    ...
    def test_extracts_records_from_array(self, extractor: JsonExtractor, tmp_path: Path) -> None:
    ...
    def test_extra_fields_become_metadata(self, extractor: JsonExtractor, tmp_path: Path) -> None:
    ...
    def test_none_valued_fields_dropped_from_metadata(self, extractor: JsonExtractor, tmp_path: Path) -> None:
    ...
    def test_text_and_id_field_overrides(self, extractor: JsonExtractor, tmp_path: Path) -> None:
    ...
    def test_metadata_fields_whitelist(self, extractor: JsonExtractor, tmp_path: Path) -> None:
    ...
    def test_empty_text_skipped_with_text_field_override(self, extractor: JsonExtractor, tmp_path: Path) -> None:
        """Empty/missing text is still skipped under a custom text field."""
        _write_json(
            tmp_path / "data.json",
            [{"body": ""}, {"text": "wrong field"}, {"body": "ok"}],
        )
        docs = list(extractor.extract(tmp_path, "src", "med", metadata={"text_field": "body"}))
        assert [d.text for d in docs] == ["ok"]

    def test_single_object_with_overrides(self, extractor: JsonExtractor, tmp_path: Path) -> None:
    ...
    def test_skips_records_without_text(self, extractor: JsonExtractor, tmp_path: Path) -> None:
    ...
    def test_top_level_object_is_treated_as_single_record(self, extractor: JsonExtractor, tmp_path: Path) -> None:
    ...
    def test_invalid_json_is_skipped(self, extractor: JsonExtractor, tmp_path: Path) -> None:
        """Files with bad JSON are skipped with a warning."""
        (tmp_path / "bad.json").write_text("{not json", encoding="utf-8")
        docs = list(extractor.extract(tmp_path, "src", "med"))
        assert docs == []

    def test_recursive_scan(self, extractor: JsonExtractor, tmp_path: Path) -> None:
