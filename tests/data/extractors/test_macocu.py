...
class TestMacocuExtractor:
    ...
    def test_extracts_one_doc_per_doc_element(self, extractor: MacocuExtractor, tmp_xml: Path) -> None:
        """One Document per non-empty <doc>; empty ones are skipped."""
        docs = list(extractor.extract(tmp_xml, "macocu", "web"))
        assert len(docs) == 2

    def test_joins_paragraphs_with_newline(self, extractor: MacocuExtractor, tmp_xml: Path) -> None:
        """Multiple <p> children are joined with a newline."""
        docs = list(extractor.extract(tmp_xml, "macocu", "web"))
        assert docs[0].text == "Dober dan.\nKako ste?"

    def test_doc_id_from_doc_attribute(self, extractor: MacocuExtractor, tmp_xml: Path) -> None:
        """The doc_id is read from the <doc> id attribute."""
        docs = list(extractor.extract(tmp_xml, "macocu", "web"))
        assert docs[0].doc_id == "macocu.sl.1"
        assert docs[1].doc_id == "macocu.sl.2"

    def test_metadata_includes_doc_attributes(self, extractor: MacocuExtractor, tmp_xml: Path) -> None:
    ...
    def test_source_and_domain(self, extractor: MacocuExtractor, tmp_xml: Path) -> None:
    ...
    def test_no_annotations(self, extractor: MacocuExtractor, tmp_xml: Path) -> None:
        """MaCoCu Documents carry plain text only."""
        docs = list(extractor.extract(tmp_xml, "macocu", "web"))
        for doc in docs:
            assert doc.annotations is None

    def test_skips_invalid_xml(self, extractor: MacocuExtractor, tmp_path: Path) -> None:
    ...
    def test_recursive_directory_scan(self, extractor: MacocuExtractor, tmp_path: Path) -> None:
