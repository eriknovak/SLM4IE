...
class TestMetadataLookupBasics:
    ...
    def test_lookup_hit_renames_fields(self, tmp_path: Path) -> None:
        ...
        assert lookup.get("kas-10000") == {"title": "First", "udc_code": "005"}
    ...
    def test_lookup_miss_returns_empty(self, tmp_path: Path) -> None:
        ...
        assert lookup.get("does-not-exist") == {}
    ...
    def test_unconfigured_columns_dropped(self, tmp_path: Path) -> None:
        ...
        assert "cerif" not in lookup.get("kas-10000")


class TestMetadataLookupNa:
    ...
    def test_empty_string_treated_as_na(self, tmp_path: Path) -> None:
        ...
        assert lookup.get("kas-10001") == {}


class TestMetadataLookupSplits:
    ...
    def test_pipe_split_produces_list(self, tmp_path: Path) -> None:
        ...
        assert lookup.get("kas-10000") == {"cerif": ["P000", "T270"]}
    ...
    def test_split_drops_na_parts(self, tmp_path: Path) -> None:
        ...
        assert lookup.get("x") == {"foo": ["a", "b"]}


class TestMetadataLookupKeyDerivation:
    """Mapping file paths to lookup keys."""
    ...
    def test_filename_stem_default(self, tmp_path: Path) -> None:
        ...
        assert lookup.get_for_path(Path("kas-10000.xml")) == {"title": "First"}

    def test_regex_extracts_group_one(self, tmp_path: Path) -> None:
        """``key_pattern`` group 1 becomes the actual lookup key."""
        content = textwrap.dedent("""\
            id\tfield
            10000\tphysics
        """)
        ...
        assert lookup.get_for_path(Path("oss-10000.conllu")) == {"field": "physics"}

    def test_regex_no_match_returns_empty(self, tmp_path: Path) -> None:
        """When the regex doesn't match, lookup returns ``{}``."""
        path = _write_tsv(tmp_path)
        lookup = MetadataLookup(
            path=path,
            key_column="id",
        ...
        assert lookup.get_for_path(Path("kas-10000.xml")) == {}


class TestMetadataLookupGzip:
    """Gzip-aware reads."""
    ...
    def test_loads_gzipped_tsv(self, tmp_path: Path) -> None:
        ...
        assert lookup.get("kas-10001") == {"title": "Second"}


class TestMetadataLookupErrors:
...
class TestMetadataLookupFromConfig:
    ...
    def test_from_config_resolves_relative_path(self, tmp_path: Path) -> None:
        ...
        lookup = MetadataLookup.from_config(
            input_dir=tmp_path,
            cfg={
                "path": "sub/meta.tsv",
                "key_column": "id",
        ...
        assert lookup.get("kas-10000") == {"title": "First"}
