...
from slm4ie.data.extract import _chunk_files
...
class TestTextExtractorDocIds:
    ...
    def test_doc_id_is_file_relative_with_block_index(self, tmp_path: Path) -> None:
    ...
    def test_whitespace_only_blocks_do_not_consume_an_index(self, tmp_path: Path) -> None:
        """Blocks that strip to nothing are skipped without leaving a gap."""
        (tmp_path / "sl.txt").write_text("Prvi.\n\n   \n\nDrugi.\n", encoding="utf-8")
    ...
    def test_serial_and_sharded_ids_are_identical(self, tmp_path: Path) -> None:
        ...
        for chunk in _chunk_files(files, 3):
            sharded.extend(_ids(list(extractor.extract_files(chunk, "cc100", "web", tmp_path))))

        assert sharded == serial

    def test_doc_id_falls_back_to_filename_outside_input_dir(self, tmp_path: Path) -> None:
        ...
        docs = list(TextExtractor().extract_files([outside / "sl.txt"], "cc100", "web", input_dir))
