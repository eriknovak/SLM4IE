...
class TestHuggingFaceExtractor:
    ...
    def test_extracts_text_column(self, tmp_path: Path) -> None:
        ...
        (tmp_path / "sl").mkdir()
        with patch("slm4ie.data.extractors.huggingface.load_from_disk") as mock_load:
            mock_ds = MagicMock()
            ...
            docs = list(extractor.extract(tmp_path, source="fineweb2", domain="web"))
    ...
    def test_handles_dataset_dict(self, tmp_path: Path) -> None:
        ...
        with patch("slm4ie.data.extractors.huggingface.load_from_disk") as mock_load:
            ...
            mock_split.__iter__ = MagicMock(return_value=iter([{"text": "Train row."}]))
    ...
    def test_skips_failed_config_dir(self, tmp_path: Path) -> None:
        ...
        with patch("slm4ie.data.extractors.huggingface.load_from_disk") as mock_load:
            mock_ds = MagicMock()
            mock_ds.__iter__ = MagicMock(return_value=iter([{"text": "Good row."}]))
            mock_ds.column_names = ["text"]
...
def _save_config(root: Path, config: str, rows: List[Dict[str, Any]]) -> None:
...
class TestHuggingFaceDocIds:
    ...
    def test_non_string_id_is_coerced(self, tmp_path: Path) -> None:
        ...
        _save_config(tmp_path, "sl", [{"text": "Prvi.", "id": 7}, {"text": "Drugi.", "id": 8}])
    ...
    def test_positional_fallback_for_single_dataset(self, tmp_path: Path) -> None:
        """A bare Dataset with no key column gets `<config>:<row>` ids."""
        _save_config(tmp_path, "sl", [{"text": "Prvi."}, {"text": "Drugi."}])

        docs = list(HuggingFaceExtractor().extract(tmp_path, "c4", "web"))

        assert [d.doc_id for d in docs] == ["sl:00000000", "sl:00000001"]

    def test_positional_fallback_includes_split_for_dataset_dict(self, tmp_path: Path) -> None:
    ...
    def test_positional_ids_are_unique_across_configs(self, tmp_path: Path) -> None:
