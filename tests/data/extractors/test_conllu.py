...
class TestConlluExtractor:
    ...
    def test_one_newdoc_two_sentences_yields_single_document(self, tmp_path: Path) -> None:
    ...
    def test_one_newdoc_flat_tokens_across_sentences(self, tmp_path: Path) -> None:
    ...
    def test_sent_id_prefix_splits_solar_style_essays(self, tmp_path: Path) -> None:
        """No newdoc + hierarchical sent_ids → split by leading prefix."""
        docs = _extract(tmp_path, SOLAR_STYLE_TWO_ESSAYS, filename="solar-orig.conllu")
        assert len(docs) == 2
        ...
        assert docs[0].text == ("Prva poved prvega eseja.\nDruga poved prvega eseja.")
        assert docs[1].text == "Prva poved drugega eseja."

    def test_newdoc_marker_overrides_sent_id_prefix_heuristic(self, tmp_path: Path) -> None:
    ...
    def test_source_and_domain(self, tmp_path: Path) -> None:
        ...
        extractor = ConlluExtractor()
        docs = list(extractor.extract(tmp_path, source="ssj500k", domain="news"))
    ...
    def test_processes_multiple_files(self, tmp_path: Path) -> None:
        ...
        extractor = ConlluExtractor()
        docs = list(extractor.extract(tmp_path, source="test", domain="test"))
        assert len(docs) == 2
    ...
    def test_text_reconstructed_when_comment_absent(self, tmp_path: Path) -> None:
    ...
    def test_space_after_detected_among_other_misc(self, tmp_path: Path) -> None:
    ...
    def test_directory_named_like_conll_is_skipped(self, tmp_path: Path) -> None:
        ...
        extractor = ConlluExtractor()
        docs = list(extractor.extract(tmp_path, source="kzb", domain="x"))
