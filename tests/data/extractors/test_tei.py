...
class TestTeiUtteranceExtraction:
    ...
    def test_one_document_per_utterance(self, extractor: TeiExtractor, tmp_utterance: Path) -> None:
        """Each <u> element becomes its own Document."""
        docs = list(extractor.extract(tmp_utterance, "parlamint_si", "parliamentary"))
        assert len(docs) == 2

    def test_utterance_text_joins_sentences(self, extractor: TeiExtractor, tmp_utterance: Path) -> None:
    ...
    def test_doc_id_from_utterance_xml_id(self, extractor: TeiExtractor, tmp_utterance: Path) -> None:
        """doc_id comes from <u>'s xml:id, not from individual <s>."""
        docs = list(extractor.extract(tmp_utterance, "parlamint_si", "parliamentary"))
        assert docs[0].doc_id == "u1"
        assert docs[1].doc_id == "u2"

    def test_utterance_metadata_carries_speaker(self, extractor: TeiExtractor, tmp_utterance: Path) -> None:
    ...
    def test_utterance_annotations_flat_with_sentence_spans(self, extractor: TeiExtractor, tmp_utterance: Path) -> None:
...
class TestTeiPerFileFallback:
    ...
    def test_one_document_per_file(self, extractor: TeiExtractor, tmp_kas: Path) -> None:
        """No <u> in the file → exactly one Document per file."""
        docs = list(extractor.extract(tmp_kas, "kas", "academic"))
        assert len(docs) == 1

    def test_doc_id_from_file_stem(self, extractor: TeiExtractor, tmp_kas: Path) -> None:
        """doc_id falls back to the source file's stem when no <u>."""
        docs = list(extractor.extract(tmp_kas, "kas", "academic"))
        assert docs[0].doc_id == "kas"

    def test_aggregated_text_and_annotations(self, extractor: TeiExtractor, tmp_kas: Path) -> None:
...
class TestTeiJoinSpacing:
    ...
    def test_kas_period_attached(self, extractor: TeiExtractor, tmp_kas: Path) -> None:
    ...
    def test_all_join_values(self, extractor: TeiExtractor, tmp_path: Path) -> None:
    ...
    def test_space_after_persisted_in_annotation_line(self, extractor: TeiExtractor, tmp_kas: Path) -> None:
    ...
    def test_join_inside_name_wrapper(self, extractor: TeiExtractor, tmp_path: Path) -> None:
...
class TestTeiPlainParagraphs:
    ...
    def test_one_document_per_paragraph(self, extractor: TeiExtractor, tmp_plain: Path) -> None:
    ...
    def test_processes_multiple_xml_files(self, extractor: TeiExtractor, tmp_path: Path) -> None:
    ...
    def test_handles_nested_dirs(self, extractor: TeiExtractor, tmp_path: Path) -> None:
...
class TestTeiMetadata:
    ...
    def test_metadata_attached_via_filename_stem(self, extractor: TeiExtractor, tmp_path: Path) -> None:
    ...
    def test_metadata_attached_to_plain_paragraphs(self, extractor: TeiExtractor, tmp_path: Path) -> None:
    ...
    def test_metadata_layers_under_utterance_attributes(self, extractor: TeiExtractor, tmp_path: Path) -> None:
    ...
    def test_no_metadata_kwarg_keeps_empty_dict(self, extractor: TeiExtractor, tmp_kas: Path) -> None:
...
class TestParsePlainBodyScoped:
    ...
    def test_empty_body_segment_yields_no_documents(self, extractor: TeiExtractor, tmp_path: Path) -> None:
        ...
        """
        (tmp_path / "GF0011059-7-dedup.xml").write_text(_GIGAFIDA_EMPTY_SEGMENT, encoding="utf-8")
        docs = list(extractor.extract(tmp_path, "gigafida", "mixed"))
        assert docs == []

    def test_header_paragraphs_never_extracted(self, extractor: TeiExtractor, tmp_path: Path) -> None:
        """Only `<body>` paragraphs become documents; header `<p>` is ignored."""
        (tmp_path / "doc.xml").write_text(_PLAIN_TEI_WITH_HEADER_P, encoding="utf-8")
...
class TestStreamingLargeFile:
    ...
    def test_streaming_matches_full_dom_per_file(self, tmp_path: Path) -> None:
        ...
        full = list(_parse_annotated_per_file(root, "kas", "academic", doc_id="kas", extra_metadata={}))
    ...
    def test_large_gate_streaming_equiv_per_file(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        normal = [(x.to_jsonl_line(), x.to_annotation_line()) for x in ext.extract(d, "kas", "academic")]
        monkeypatch.setattr(tei_module, "_LARGE_FILE_BYTES", 0)
        streamed = [(x.to_jsonl_line(), x.to_annotation_line()) for x in ext.extract(d, "kas", "academic")]
        assert streamed == normal
        assert len(normal) == 1

    def test_large_gate_utterance_fallback(self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
