...
The reporting counterpart to `slm4ie.utils.mlflow` (the write side): each
experiment's `analysis.py` and other consumers call these to pull runs,
metrics, and artifacts from the MLflow store and shape them into pandas
frames. Keeping the queries here (rather than inline in an analysis script)
keeps those scripts thin and lets the shaping logic be unit-tested against a
temporary store.
...
PRETRAIN_EXPERIMENT = "slm4ie/data/curate"
...
def contamination_risk(
    *,
    ...
    leakage risk -- most damaging for `held_out` task splits.
