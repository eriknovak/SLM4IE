...
def download(config: DatasetConfig, output_dir: Path, force: bool) -> DownloaderResult:
    ...
    for cfg_name in configs:
        try:
            ...
            if staging.exists():
                logger.info(
                    "Removing orphan staging dir for '%s'",
                    cfg_name,
                )
            ...
            failed.append((cfg_name, _augment_with_note(exc, config.note)))
