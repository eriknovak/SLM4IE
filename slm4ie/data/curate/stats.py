...
def _build_bundle(
    *,
    ...
    return {
        ...
        f"word_freq_top_{top_k_words}": [[tok, int(count)] for tok, count in word_freq.most_common(top_k_words)],
    }
...
class CorpusStatsReduce(PipelineStep):
    ...
    def run(self, data: DocumentsPipeline = None, rank: int = 0, world_size: int = 1) -> DocumentsPipeline:
        ...
        if not partials:
            raise FileNotFoundError(f"No partial.*.pkl files found under {self.partials_dir}; did the map step run?")
