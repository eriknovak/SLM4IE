"""Post-hoc MLflow tracking for the curation pipeline.

Reads the on-disk pipeline state after a full `--all` build and records a
single MLflow run per distinct corpus build under the `slm4ie/data/curate`
experiment. The run is keyed by a content digest of the final corpus
(`06_sentence_dedup/`) and upserted: a build whose digest already has a run is skipped
...
per-dataset / per-word statistics come from `07_statistics/aggregate.json`.
...
* The flattened resolved pretrain config as params, and `aggregate.json`,
...
#: `slm4ie/data/<stage>` lineage convention for shared pipeline stages.
DEFAULT_EXPERIMENT = "slm4ie/data/curate"
...
def read_aggregate_stats(output_dir: Path) -> Optional[Dict[str, Any]]:
    ...
    Returns:
        The parsed `07_statistics/aggregate.json`, or None when it is absent.
    """
    path = output_dir / STAGE_DIRS["statistics"] / "aggregate.json"
    if not path.exists():
        return None
...
def flatten_config(config: Any, prefix: str = "") -> Dict[str, str]:
    ...
    the pretrain config as run params.
...
def log_pretrain_run(
    ...
    *,
    ...
    Args:
        output_dir: The pretrain output root.
        config: The resolved pretrain config as a dict (logged as params).
    ...
    with ml.mlflow_run("pretrain-build", tags=tags):
        ...
        stats_dir = output_dir / STAGE_DIRS["statistics"]
        ...
        ml.log_dataset_input("pretrain/06_sentence_dedup", digest, str(final_corpus), context="produced")
