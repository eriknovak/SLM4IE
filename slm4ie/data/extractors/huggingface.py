...
class HuggingFaceExtractor(BaseExtractor):
    ...
    def extract(
        ...
        """
        del metadata
        config_dirs: List[Path] = sorted(p for p in input_dir.iterdir() if p.is_dir())

        for config_dir in config_dirs:
            try:
                ds = load_from_disk(str(config_dir))
            except Exception as exc:  # noqa: BLE001
                logger.warning("Failed to load dataset from %s: %s", config_dir, exc)
                continue

            yield from self._yield_from_dataset(ds, source, domain, config_dir.name)
    ...
    def _yield_from_dataset(
        ...
        if isinstance(ds.column_names, dict):
            ...
            for split in ds.keys():
                yield from self._yield_from_split(ds[split], source, domain, config, split)
        else:
            yield from self._yield_from_split(ds, source, domain, config, None)
