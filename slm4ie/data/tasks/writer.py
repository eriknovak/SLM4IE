...
The three task-family converters (`spans`, `sentiment`,
`superglue`) all share the same output convention — one gzipped
...
def hash_split(
    ...
    buckets `[0, train_pct)` go to `train`, `[train_pct, train_pct + val_pct)`
    go to `val`, and the remainder to `test`.
