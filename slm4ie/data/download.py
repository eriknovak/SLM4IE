...
class DatasetDownloadError(Exception):
    ...
    def __init__(
        ...
        unit_summary = ", ".join(
            f"{unit}: {exc.__class__.__name__}: {str(exc).splitlines()[0]}" for unit, exc in self.failed
        )
        super().__init__(f"{dataset_key}: {len(self.failed)}/{n_total} sub-units failed ({unit_summary})")


def _augment_with_note(exc: BaseException, note: Optional[str]) -> BaseException:
    ...
    if not note:
        return exc
    return RuntimeError(f"{exc.__class__.__name__}: {exc} — {note.strip()}")
...
def _http_download(config: DatasetConfig, output_path: Path, force: bool) -> DownloaderResult:
    """Dispatch to the http source downloader via module attribute lookup."""
    return http_source.download(config, output_path, force)


def _hf_download(config: DatasetConfig, output_path: Path, force: bool) -> DownloaderResult:
...
def _validate_selection(
    ...
    for key, config in selected.items():
        ...
        if explicit and config.manual and not _dir_has_files(Path(base_output_dir) / config.output_dir):
            problems.append(f"{key}: explicitly selected but requires manual download" + _note_suffix(config.note))
...
def download_datasets(
    ...
    if dataset_keys:
        ...
        if only_benchmarks:
            selected = {k: v for k, v in selected.items() if v.role != "pretrain"}
        elif exclude_benchmarks:
            selected = {k: v for k, v in selected.items() if v.role == "pretrain"}

    _validate_selection(selected, base_output_dir, explicit=bool(dataset_keys))
    ...
    if failures:
        ...
        for key, exc in failures:
            if isinstance(exc, DatasetDownloadError):
                ...
                lines.append(f"  - {key}: {exc.__class__.__name__}: {exc}")
